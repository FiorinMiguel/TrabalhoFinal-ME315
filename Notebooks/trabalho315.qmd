...
```{=html}
<p style="font-size:0.6em; color:#555;">
Nomear variáveis ou colunas seguindo boas práticas é essencial para garantir consistência, clareza e evitar erros no código. Usar nomes padronizados — sem espaços, acentos ou caracteres especiais, com letras minúsculas e “_” entre palavras — torna o trabalho mais organizado, facilita chamadas em funções e assegura compatibilidade entre diferentes linguagens e ferramentas, resultando em um fluxo de trabalho mais simples e robusto.
</p>
```
...
df_subset = select(dataset_eletric_cars, [:Brand, :Model, :Battery_capacity_kWh])

# Também é possível selecionar intervalos de colunas:
df_subset2 = select(dataset_eletric_cars, Between(:Brand, :Top_Speed_kmh))

# Seleção por posição:
df_subset3 = select(dataset_eletric_cars, 1:3)

# Deselecionar colunas (remover):
df_subset4 = select(dataset_eletric_cars, Not(["Model", "Brand"]))
...
# Seleção simples
df_subset <- dataset_eletric_cars %>%
  select(Brand, Model, Battery_capacity_kWh)

# Seleção por intervalo de colunas
df_subset2 <- dataset_eletric_cars %>%
  select(Brand:Top.Speed.kmh)

# Seleção por padrão de nome
df_subset3 <- dataset_eletric_cars %>%
  select(starts_with("Bat"))

# Deselecionar colunas
df_subset4 <- dataset_eletric_cars %>%
  select(-Model, -Brand)
...
# Seleção simples
df_subset = dataset_eletric_cars[["Brand", "Model", "Battery_capacity_kWh"]]

# Seleção de intervalo pelo índice
df_subset2 = dataset_eletric_cars.iloc[:, 0:3]  # primeiras 3 colunas

# Seleção usando padrão de nome
df_subset3 = dataset_eletric_cars.filter(regex="^Bat")

# Deselecionar colunas
df_subset4 = dataset_eletric_cars.drop(columns=["Model", "Brand"])
...
df_transformed = transform(
    ...
    :Battery_capacity_kWh => (x -> x .* 1000) => :battery_wh,
    :Top_Speed_kmh => (x -> x ./ 3.6) => :top_speed_ms
)

# Aplicando múltiplas funções a uma mesma coluna
df_transformed2 = transform(
    dataset_eletric_cars,
...
df_transformed <- dataset_eletric_cars %>%
  mutate(
    battery_wh = Battery_capacity_kWh * 1000,
    top_speed_ms = Top.Speed.kmh / 3.6
  )

# Aplicando múltiplas transformações a uma mesma coluna
df_transformed2 <- dataset_eletric_cars %>%
  mutate(
    battery_mean = mean(Battery_capacity_kWh),
    battery_sd   = sd(Battery_capacity_kWh)
  )
...
# Criando novas colunas
df_transformed = dataset_eletric_cars.assign(
    battery_wh=lambda df: df["Battery_capacity_kWh"] * 1000,
    top_speed_ms=lambda df: df["Top Speed kmh"] / 3.6
)

# Aplicando múltiplas funções a uma única coluna
df_transformed2 = dataset_eletric_cars.copy()
df_transformed2["battery_mean"] = dataset_eletric_cars["Battery_capacity_kWh"].mean()
df_transformed2["battery_sd"]   = dataset_eletric_cars["Battery_capacity_kWh"].std()
